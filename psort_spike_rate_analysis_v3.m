...
%
% Analysed results are placed in array 'cs_ss_be_du_af_...', which columns are organized in sequence as: 
% [1 2 3 4 5 6] column sequence corresponds to 
% [rate_cs_before rate_cs_during rate_cs_after rate_ss_before rate_ss_during rate_ss_after]
% [7] column is difference between 2 and 1 columns, [2]-[1]
% [8] column is difference between 5 and 4 columns, [5]-[4]
%
% The *.psort files must be in one folder
% First type pathway to the *.psort files in 'Folder' for "Sural", "Tibial" or "CP";
% choose "cs_ss_be_du_af_30_train_Sural = []" for "Sural", "Tibial" or "CP"
% and "cs_ss_be_du_af_30_train_Sural = vertcat(cs_ss_be_du_af_30_train_Sural, new_cs_ss_be_du_af)" for "Sural", "Tibial" or "CP"
% then uncomment one of the "-Sural-", "-Tibial-" or "-Common Peroneal-" and comment rest;
...
% Folder = '/media/murat/T7 Touch/spike_analysis_before_during_after/psorted/Tibial_10_stim/';
% Folder = '/media/murat/T7 Touch/spike_analysis_before_during_after/psorted/CP_10_stim/';
...
% cs_ss_be_du_af_30_train_Tibial = [];
% cs_ss_be_du_af_30_train_CP = [];
...
% cs_ss_be_du_af_30_train_Tibial = vertcat(cs_ss_be_du_af_30_train_Tibial, new_cs_ss_be_du_af);
% cs_ss_be_du_af_30_train_CP = vertcat(cs_ss_be_du_af_30_train_CP, new_cs_ss_be_du_af);
...
CS_neg_pos_percent = [CS_neg_percent CS_pos_percent];
SS_neg_pos_percent = [SS_neg_percent SS_pos_percent];

keep cs_ss_be_du_af_30_train_Sural Folder FileList CSneg CSpos CSzer SSneg SSpos SSzer numRows numCols ...
    CS_neg_percent CS_pos_percent SS_neg_percent SS_pos_percent CS_neg_pos_percent SS_neg_pos_percent;
%*********************************************************************************************


% 
% %***************************       -Tibial-        *********************************
% cs_ss_be_du_af_30_train_Tibial(:,7) = cs_ss_be_du_af_30_train_Tibial(:,2)-cs_ss_be_du_af_30_train_Tibial(:,1);
% CSneg = sum(cs_ss_be_du_af_30_train_Tibial(:,7) < 0);
% CSpos = sum(cs_ss_be_du_af_30_train_Tibial(:,7) > 0);
% CSzer = sum(cs_ss_be_du_af_30_train_Tibial(:,7) == 0);
% 
% cs_ss_be_du_af_30_train_Tibial(:,8) = cs_ss_be_du_af_30_train_Tibial(:,5)-cs_ss_be_du_af_30_train_Tibial(:,4);
% SSneg = sum(cs_ss_be_du_af_30_train_Tibial(:,8) < 0);
% SSpos = sum(cs_ss_be_du_af_30_train_Tibial(:,8) > 0);
% SSzer = sum(cs_ss_be_du_af_30_train_Tibial(:,8) == 0);
% 
% [numRows,numCols] = size(cs_ss_be_du_af_30_train_Tibial);
% CS_neg_percent = round(CSneg*100/numRows,0);
% CS_pos_percent = round(CSpos*100/numRows,0);
% SS_neg_percent = round(SSneg*100/numRows,0);
% SS_pos_percent = round(SSpos*100/numRows,0);
...
% SS_neg_pos_percent = [SS_neg_percent SS_pos_percent];
% 
% keep cs_ss_be_du_af_30_train_Tibial Folder FileList CSneg CSpos CSzer SSneg SSpos SSzer numRows numCols ...
%     CS_neg_percent CS_pos_percent SS_neg_percent SS_pos_percent CS_neg_pos_percent SS_neg_pos_percent;
% %****************************************************************************************
...
% 
% %**********************************     -Common Peroneal-       *****************************
% cs_ss_be_du_af_30_train_CP(:,7) = cs_ss_be_du_af_30_train_CP(:,2)-cs_ss_be_du_af_30_train_CP(:,1);
% CSneg = sum(cs_ss_be_du_af_30_train_CP(:,7) < 0);
% CSpos = sum(cs_ss_be_du_af_30_train_CP(:,7) > 0);
% CSzer = sum(cs_ss_be_du_af_30_train_CP(:,7) == 0);
% 
% cs_ss_be_du_af_30_train_CP(:,8) = cs_ss_be_du_af_30_train_CP(:,5)-cs_ss_be_du_af_30_train_CP(:,4);
% SSneg = sum(cs_ss_be_du_af_30_train_CP(:,8) < 0);
% SSpos = sum(cs_ss_be_du_af_30_train_CP(:,8) > 0);
% SSzer = sum(cs_ss_be_du_af_30_train_CP(:,8) == 0);
% 
% [numRows,numCols] = size(cs_ss_be_du_af_30_train_CP);
% CS_neg_percent = round(CSneg*100/numRows,0);
% CS_pos_percent = round(CSpos*100/numRows,0);
% SS_neg_percent = round(SSneg*100/numRows,0);
% SS_pos_percent = round(SSpos*100/numRows,0);
...
% SS_neg_pos_percent = [SS_neg_percent SS_pos_percent];
% 
% keep cs_ss_be_du_af_30_train_CP Folder FileList CSneg CSpos CSzer SSneg SSpos SSzer numRows numCols ...
%     CS_neg_percent CS_pos_percent SS_neg_percent SS_pos_percent CS_neg_pos_percent SS_neg_pos_percent;
% %*********************************************************************************************
% 
